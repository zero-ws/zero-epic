...
/**
 * 新容器，带内置 SPI 绑定的专用异常信息处理程序，用于桥接 {@link io.horizon.exception.AbstractException} 实现消息的
 * 可定制化，包括不同上下文环境的消息定制化部分。
 * <pre><code>
 *     1. 在原始基础上追加一层，每个异常构造构造时构造对应的 {@link KFailure}
 *     2. 标准环境
 *        OSGI环境
 *     3. 最终目的是封装 {@link io.horizon.spi.HorizonIo} 的基础容器追加
 *        输出部分：
 *        - code
 *        - info
 *        - message
 ...
 *        - params
 * </code></pre>
 * 此类中不包含 {@link io.horizon.eon.em.web.HttpStatusCode} 部分的内容，该内容由 {@link io.horizon.exception.WebException}
 * 负责构造。
 *
